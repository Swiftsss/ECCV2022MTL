...
class ResNet_backbone(nn.Module):
    def __init__(self, args) -> None:
        super().__init__()
        self.feature_extractor = get_backbone_from_name('resnet18')
        self.stream = TS_TwoT_model(args)
    def forward(self, x, init_state=None):
        feature_maps = self.feature_extractor(x)
        outputs, new_init_state = self.stream(feature_maps, init_state)
        return outputs, new_init_state
    def set_gpu(self):
        self.stream.set_gpu()

class TS_TwoT_model(nn.Module):
    def __init__(self, args) -> None:
        super().__init__()
        self.feature_extractor = get_backbone_from_name('inception_v3', pretrained=True, remove_classifier=True)
        self.EmotionNet = EmotionNet(args, 512)
    ...
    def forward(self, feature_maps, init_state=None):
