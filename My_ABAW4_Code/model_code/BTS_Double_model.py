...
class BTS_Double_model(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.backward_temporal_u = {}
        self.backward_temporal_u['VA'] = nn.Parameter(torch.tensor(5.))
        self.backward_temporal_u['EXPR'] = nn.Parameter(torch.tensor(5.))
        self.backward_temporal_u['AU'] = nn.Parameter(torch.tensor(5.))
    ...
    def temporal_smooth(self, outputs, init_state):
        new_outputs = {}
        new_backward_outputs = {}
        tasks = ['EXPR', 'AU', 'VA']
        for task in tasks:
            new_outputs[task] = torch.zeros_like(outputs[task])
            new_backward_outputs[task] = torch.zeros_like(outputs[task])

        index = len(outputs['VA']) - 2
        while index > -1:
            for task in tasks:
                new_backward_outputs[task][index] = (outputs[task][index]+self.backward_temporal_u[task]*outputs[task][index+1])/(1+self.backward_temporal_u[task])
            index -= 1
        ...
        while index < len(outputs['VA']):
            for task in tasks:
                ...
                new_outputs[task][index] = (new_outputs[task][index] + new_backward_outputs[task][index])/2
            index += 1
